...
def get_links(url):
    ...
    all_links = []
    info = soup.find_all('div', {'class': 'dyear'})
    for i in info:
        links = ['https://russkayarech.ru'+j for j in
...
def write_to_txt(path, text):
    with open(f'/Users/mariabocharova/PycharmProjects/Thesis/texts/Русская_речь_2/{path}',
...
def make_file(url):
    ...
    for elem in info:
        ...
        try:
            ...
            link = 'https://russkayarech.ru' + re.findall(r'a href="(.+?)"', elem)[0]
            ...
            resource = f"«Русская речь. {date}. №{issue}»"
            block = {'link': download_pdf_link(link),
...
for elem in get_links('https://russkayarech.ru/ru/archive'):
    make_file(elem)