...
def make_file(url):
    ...
    pdf_links_raw = soup.find_all('a', {'target': '_blank'},
                                  href=re.compile(r'//iling-ran\.ru/library/ural-altaic/.+\d{1,}\.pdf'))
    pdf_links = ['https:' + re.findall(r'href="(.+?)"', str(i))[0] for i in pdf_links_raw]
    dates = [re.findall(r'\d\d\d\d', str(i))[0] for i in pdf_links]
    issues = [re.findall(r'(\d{1,})\.pdf', str(i))[0] for i in pdf_links]
    for i in range(len(pdf_links_raw)):
        try:
            ...
            resource = title
            block = {'link': pdf_links[i], 'path': path,
                     'title': title, 'author': '-',
                     'date': dates[i], 'resource': resource}
...
make_file('https://iling-ran.ru/web/ru/publications/journals/ural-altai/issues')
